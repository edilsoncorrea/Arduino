...
my $arduSocket = IO::Socket::INET->new(	
									Proto 		=> 'tcp',
									PeerAddr 	=>	'127.0.0.1',
									PeerPort 	=> $ser_port
...
sub parseMessage
{
	...
	$bearing_error = sprintf("%1.1f", $bearing_error);
	$pitch_out = sprintf("%2.3f", $pitch_out);
	if ($count){
		#printf("roll: %.3f,  pitch: %.3f,   thr: %.3f, rud: %.3f \n", $roll_out,$pitch_out,$throttle_out,$rudder_out);
		#print "pitch:$pitch_out, wp_dist:$wp_distance \tbearing_err:$bearing_error \talt_error:$alt_error \tenergy_err:$energy_error \tWP:$wp_index \tmode:$control_mode_alpha\r";
		print "wp_dist:$wp_distance \tbearing_err:$bearing_error \talt_error:$alt_error \ttarget_alt:$energy_error \tWP:$wp_index \tmode:$control_mode_alpha\t\t\t\r";
		#print "WP:$wp_index wp_dist:$wp_distance eggDist:$int_4 \t\r";
