...
{
  /* Setup encoders */
  pinModeFast(leftEncoder1,  INPUT);
  pinModeFast(leftEncoder2,  INPUT);
  pinModeFast(rightEncoder1, INPUT);
  pinModeFast(rightEncoder2, INPUT); 
  
  attachInterrupt(0, leftEncoder,  CHANGE); // pin 2
  attachInterrupt(1, rightEncoder, CHANGE); // pin 3
  ...
  //while (! VerificaControleLigado()); // Halt
...
void InicializaConfPinos()
{
  ...
  digitalWriteFast(PoM1, LOW) ;   // Both motor with same polarity
  digitalWriteFast(PoM2, LOW) ;
  
  analogWrite(PWM1, 0);   // Stop both motors => ValMx = 0
  analogWrite(PWM2, 0);    

  digitalWriteFast(ENA, HIGH);
  digitalWriteFast(ENB, HIGH);    
}
